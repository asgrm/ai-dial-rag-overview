...
class MicrowaveRAG:
    ...
    def _create_new_index(self) -> VectorStore:
        ...
        print('creating text loader')
        text_loader = TextLoader(file_path=manual_path, encoding="utf-8")

        #  2. Load documents with loader
        print('text_loader.load() call')
        ...
        #       - chunk_overlap=50
        #       - separators=["\n\n", "\n", "."]
        print('creation of RecursiveCharacterTextSplitter')
        ...
        print('Create vectorstore from documents')
        ...
        print('Save indexed data locally with index name "microwave_faiss_index"')
    ...
    def retrieve_context(self, query: str, k: int = 4, score=0.3) -> str:
        ...
        relevant_docs = self.vectorstore.similarity_search_with_relevance_scores(k=k, score_threshold=score, query=query)
        ...
        for doc, doc_score in relevant_docs:
            ...
            print(f"\n--- (Document relevance score: {doc_score:.4f}) ---")
